...
[← Интеграция с API](API_RU.md) · [К документации](README_RU.md) · [Участие в разработке →](CONTRIBUTING_RU.md)
...
`Ввод PIN` - приложение пробует восстановить ключ шифрования из сохраненных данных и введенного PIN. Если PIN верный, защищенное хранилище становится доступно. Если PIN неверный, обновляется счетчик ошибок. На этом экране также доступна явная кнопка выхода, поэтому для удаления данных с устройства не нужно помнить PIN.
...
`Выход с PIN-экрана` использует тот же сценарий локальной auth-блокировки, что и нарушение целостности данных при настройке PIN. Сначала приложение пытается завершить текущую серверную сессию, затем очищает все локальные данные. При успешном logout локальная отметка блокировки удаляется и открывается экран входа. Если браузер offline или запрос завершился ошибкой, локальные данные всё равно удаляются и сохраняется отметка локальной блокировки; приложение повторит серверный logout после восстановления сети.
...
| IndexedDB через `idb-keyval`         | `viatrade_security_local_auth_blocked`     | Отметка незавершенной локальной auth-блокировки       | Не дает открыть приватные страницы, пока повторяется backend logout            | Открытая отметка; удаляется после успешного logout или следующего успешного входа                  |
...
Пока приложение заблокировано PIN-кодом, обычные доменные API-запросы и refresh retry блокируются. Это нужно, чтобы закрытое приложение не восстанавливало и не обновляло чувствительные данные в фоне. Экран ввода PIN при этом предоставляет явное действие выхода, которое запускает сценарий локальной auth-блокировки.

При наличии сети это действие завершает текущую серверную сессию, очищает все локальные данные и открывает экран входа. Если пользователь offline или запрос logout завершается ошибкой, frontend очищает локальные данные и сохраняет отметку локальной блокировки. Он не может удалить `Secure; HttpOnly` cookies и не должен утверждать, что серверная сессия завершена; попытка завершения сессии будет повторена после восстановления сети.
...
- [Интеграция с API](API_RU.md) — обработка запросов и сгенерированные клиенты.
- [Конфигурация](CONFIGURATION_RU.md) — настройка PWA и API URL.
- [Участие в разработке](CONTRIBUTING_RU.md) — соглашения по хранилищу и обработке ошибок.
